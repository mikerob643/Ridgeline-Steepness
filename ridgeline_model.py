...
# find first branch
branch=200 #threshold of instability that causes branching # perron root of branching 

firstbranchpe=1.78*((1/3)*(branch*(D/K)**(1/(m+(1/2)))))**(.49)+xc #perron root of branching finding the area this branching occures and turning it into a mainstem length
...
if len(trib_spacing)>0:
 ...
 # find ridgeline slope using an ordinarly least squares regression
    ...
    results = model.fit()
    sloppe = results.params[1]
    r_squaredU = results.rsquared
    conf_int=results.conf_int(alpha=0.05)
    ci95_slope_uplift=conf_int[1]
    
    
    
  
    # caculate slope as Eq14 or Eq. 26
    
    #eq 14
    slope=(U/(beta))**(1/n)*(1/(tribspacing[-1]-tribspacing[0]))*(
        
       (tribb**a)*(L**(a*(1-tribg)))*(((((tribspacing[-1])**(a*tribg)))-(((tribspacing[0])**(a*tribg))))/a)+(((tribspacing[0])**(a)-(tribspacing[-1])**(a))/a))
    # eq 26
    
    cgeom=(-1/(a*((tribspacing[-1]/L)-(tribspacing[0]/L))))*(((tribspacing[0]/L)**a)-((tribspacing[-1]/L)**a)+((tribb**a)*((tribspacing[-1]/L))**(a*tribg))-((tribb**a)*((tribspacing[0]/L))**(a*tribg)))
