...
# loop through all basins in the landscape
    for f in range(0, np.size(basin_list), 1):
        ...
        # when converting the allbasins raster to polygons, and to then points along geometry. 
        # If this happens we kick this basin out of the analysis. 
        ...
        lowess_tight = lowess(noisy_data, in_array, frac = .05)
...
# ridge two has an inverted distance so we take care of that here 
        ...
        inverted_distance2flipped=np.flip(inverted_distance2)
        rel2flipped=np.flip(rel2)
        
        
        noisy_data=rel2flipped[rel2flipped>5+np.min(rel2flipped)]
        in_array=inverted_distance2flipped[rel2flipped>5+np.min(rel2flipped)]
        ...
        if np.size(threshold )>0:
            ...
            if np.size(this) > 0:
                ...
                testx2 = inverted_distance2flipped[(np.where(rel2flipped > threshold)[0][0]):]
                
                testy2 = rel2flipped[(np.where(rel2flipped > threshold)[0][0]):]
...
# if basin or ridgeline was skipped this removes resulting the 0 values from array. 
   ...
   # removes ridgelines that on avarage gain elevation from the headwaters to the sonut. 
    slopeofridge=slopeofridge[slopeofridge>0]
